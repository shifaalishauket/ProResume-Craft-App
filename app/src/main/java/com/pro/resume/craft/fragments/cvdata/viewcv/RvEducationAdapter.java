...
public class RvEducationAdapter extends RecyclerView.Adapter<RvEducationAdapter.ViewHolder> {
    private ArrayList<DTOEducation> mlist;
    private int tempname;
    private Context context;

    public RvEducationAdapter(ArrayList<DTOEducation> mlist, int tempname, Context context) {
        this.mlist = mlist;
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        DTOEducation currentItem = mlist.get(position);

        holder.companyname.setText(currentItem.getUniversity());


        holder.designation.setVisibility(View.GONE);

        holder.date.setText(currentItem.getStartDate() + "-" + currentItem.getEndDate());
        holder.description.setText(currentItem.getDegree());
    }
